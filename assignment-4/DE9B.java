// DE9B.java CS5125/6025 Cheng 2024
// Adaptive Arithmetic Decoding with Bi-level Images
// inverse of DE9A.java
// Usage: java DE9B < compressed > decoded.pbm
...
public class DE9B{
   ...
   boolean update(int context){  // Your code needed to complete this function.
     ...
     for (;;){ // double until larger than quarter
       if (high < half){
         ...
         codeword *= 2;
           if (inputBit() > 0) codeword |= 1;
           // two lines for updating codeword, similar to that when high < half
       }else if (low > quarter && high <= threequarters){
         low = (low * 4 - maxRange) / 2;
         high = (high * 4 - maxRange) / 2;
           codeword *= 2;
           // code for updating codeword, similar to that when high < half
       }else break;
    }
    return ret;
  }
 ...
 void uncompress(){
     int x0 = random.nextInt(600);
     int y0 = random.nextInt(600) + 1000;
     for (int i = 0; i < 16; i++){
       codeword <<= 1; 
       codeword |= inputBit();
     }
     for (int i = 0; i < height; i++){
       for (int j = 0; j < width; j++){
        ...
        bitmap[2][j + 2] = one;
        if (i >= y0 && i < y0 + 100 && j >= x0 && j < x0 + 100)
          outputBit(false); // add a random square
        else outputBit(one);
      }
